...
class TodoApp(ft.UserControl):
    def build(self):
        self.new_task = ft.TextField(
            hint_text="Crie suas tarefas aqui:", on_submit= self.add_clicked, expand= True
        )
        ...
        self.filter = ft.Tabs(
            ...
            tabs= [ft.Tab(text="Todas"), ft.Tab(text="Abertas"), ft.Tab(text="Completas")],
        )
    ...
    async def update_async(self):
        ...
        for task in self.tasks.controls:
            task.visible = (
                status == "Todas"
                or (status == "Abertas" and task.completed == False)
                or (status == "completas" and task.completed)
            )
            if not task.completed:
                count += 1
        self.items_left.value = f"{count} Abertas."
